...
# Figure S10A: down-sampled, alpha ####
# Day 2 TST ####
...
ggplot(alpha, aes(x=TCR.class,y=percentage,colour=TCR.class)) +
...
# Figure S10B: full repertoires, beta ####
# Day 2 TST ####
a <- read.csv(paste0("data/invitro-vs-mc_results_D2_full-repertoires_expanded_gr0.csv")) %>%
  select(all_of(keep))
b <- read.csv(paste0("data/invitro-vs-mc_results_D2_full-repertoires_expanded_gr1.csv")) %>%
  select(all_of(keep))
c <- read.csv(paste0("data/invitro-vs-mc_results_D2_full-repertoires_expanded_gr2.csv")) %>%
  select(all_of(keep))
d <- read.csv(paste0("data/invitro-vs-mc_results_D2_full-repertoires_expanded_gr3.csv")) %>%
  select(all_of(keep))
e <- read.csv(paste0("data/invitro-vs-mc_results_D2_full-repertoires_expanded_gr4.csv")) %>%
  select(all_of(keep))
...
ggplot(beta, aes(x=TCR.class,y=percentage,colour=TCR.class)) +
  ...
  theme(strip.background = element_blank(),
        ...
        panel.spacing = unit(0,units="line"))+
  scale_colour_manual(values = c("orange","navy")) +
  labs(y="% of total CDR3s",
       x = "Clone size") +
  ggtitle("Day 2 TST")

# Day 7 TST ####
a <- read.csv(paste0("data/invitro-vs-mc_results_D7_full-repertoires_expanded_gr0.csv")) %>%
  select(all_of(keep))
b <- read.csv(paste0("data/invitro-vs-mc_results_D7_full-repertoires_expanded_gr1.csv")) %>%
  select(all_of(keep))
c <- read.csv(paste0("data/invitro-vs-mc_results_D7_full-repertoires_expanded_gr2.csv")) %>%
  select(all_of(keep))
d <- read.csv(paste0("data/invitro-vs-mc_results_D7_full-repertoires_expanded_gr3.csv")) %>%
  select(all_of(keep))
e <- read.csv(paste0("data/invitro-vs-mc_results_D7_full-repertoires_expanded_gr4.csv")) %>%
  select(all_of(keep))
...
# Figure S10C: full repertoires, alpha ####
# Day 2 TST ####
a <- read.csv(paste0("data/invitro-vs-mc_results_D2_full-repertoires_expanded_gr0.csv")) %>%
  select(all_of(keep))
b <- read.csv(paste0("data/invitro-vs-mc_results_D2_full-repertoires_expanded_gr1.csv")) %>%
