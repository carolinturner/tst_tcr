## TCRseq_script_13: Abundance of different sets of TCRs in validation datasets
...
hla.uin <- read.csv("data/FileS4.csv") %>% pull(UIN) %>% unique()
tst.cdr3 <- tst.dat %>% 
...
# METACLONOTYPIST
mc <- read.csv("data/FileS5.csv") %>% select(regex,Vs,index) %>% unique()

# GLIPH2 pattern
gliph.pattern <- read.csv("data/FileS7.csv") %>%
  filter(!pattern == "G.T") %>%
  pull(pattern) %>%
  unique()

# GLIPH2 cdr3
gliph.cdr3 <- read.csv("data/FileS7.csv") %>% pull(CDR3s) %>% unique()
