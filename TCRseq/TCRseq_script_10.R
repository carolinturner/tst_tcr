...
# most abundant: metaclone 21 (look up Table S4 = cluster 6)
ind2 <- d1 %>% filter(count_by_mc == max(count_by_mc)) %>% pull(mc.index) %>% unique()

# alpha ####
d1 <- read.csv("data/metaclonotypist_results_down-sampled_alpha_expanded_gr0.csv") %>%
  filter(tissue == "TST_D7") %>%
  group_by(mc.index) %>%
  mutate(publicity = length(unique(sample)),
         count_by_mc = sum(duplicate_count)) %>%
...
# most public: metaclone 90 (look up in Table S4 = cluster 132)
ind1 <- d1 %>% filter(publicity == max(publicity)) %>% pull(mc.index) %>% unique()
# most abundant: metaclone 0 (look up in Table S4 = cluster 1716)
ind2 <- d1 %>% filter(count_by_mc == max(count_by_mc)) %>% pull(mc.index) %>% unique()
