...
# Figure S13A: private vs public TST enrichment (full repertoires, beta) ####
...
# plot
pS13A <- ggplot(all, aes(x=TST.day, y=percentage, colour=TCR.class))+
...
# Figure S13B: expansion d2-d7 private vs public (down-sampled, beta) ####
keep <- c("Sample","UIN","pct.PPD.only","pct.mc")
...
# plot
pS13B <- ggplot(delta, aes(x=TCR.class,y=FC.percentage,colour=TCR.class))+
...
# Figure S13C: expansion d2-d7 private vs public (full repertoires, beta) ####
keep <- c("Sample","UIN","pct.PPD.only","pct.mc")
...
# plot
pS13C <- ggplot(delta, aes(x=TCR.class,y=FC.percentage, colour = TCR.class))+
  geom_boxplot(outliers=F)+
  facet_wrap(~Clone.Size, nrow=1)+
  scale_colour_manual(values = c("orange","navy"))+
...
r2 <- ggarrange(pS13B,pS13C,
                labels = c("B","C"),
                font.label = list(size = 10, face = "bold", colour = "black"))
ggarrange(r1,r2,
          nrow = 2)
ggsave("figures/FigureS13.svg", 
