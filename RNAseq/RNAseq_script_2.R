## RNAseq_script_2: SARtools DeSeq2 analysis TST_D2 vs Saline
...
projectName <- "TST D2 vs Saline"                    # name of the project
author <- "Your name"                                # author of the statistical analysis/report

targetFile <- "SARtools_meta_TSTd2vsSal.txt"         # path to the design/target file
...
dir.create("SARtools_output_D2")
file.rename("./tables/TSTd2vssaline.complete.txt","SARtools_output_D2/TSTd2vssaline.complete.txt")
file.rename("./tables/TSTd2vssaline.up.txt","SARtools_output_D2/TSTd2vssaline.up.txt")

# up-regulated genes (FDR<0.05, log2FC>=1)
up <- read.table("SARtools_output_D2/TSTd2vssaline.up.txt", header=T) %>% 
  filter(log2FoldChange >= 1) %>%
  select(Id)

# subset tpm matrix to keep only genes up-regulated in D2 TST
library(tidyverse)
...
tpm.up <- up %>%
  ...
  na.omit() %>%
  left_join(tpm)
write.csv(tpm.up, file="tpm_D2-transcriptome.csv",row.names = F)

# make input file for IPA upstream regulator analysis
up <- read.table("SARtools_output_D2/TSTd2vssaline.up.txt", header=T) %>% 
  filter(log2FoldChange >= 1) %>%
  select(Id,log2FoldChange,padj)
write.table(up,"IPA_input_D2-vs-sal.txt",sep="\t",quote=F,row.names = F)
